...
class TeamcityCiService < CiService
  prop_accessor :teamcity_server_url, :teamcity_build_configuration_id, :username, :password
  ...
  def convert_state_teamcity_to_gitlab(build_info)
    ...
    Gitlab::AppLogger.info(
      "teamcity build_info.state = #{build_info.state} with  build_info.status = #{build_info.status} are not supported," \
      ' suppose it failed'
    )
